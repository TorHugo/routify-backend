...
@Component("ConfirmedCustomerCommunicationTemplate")
class ConfirmedCustomerCommunicationTemplate(
    ...
    override fun buildCommunicationBody(template: EmailTemplateDTO, user: UserDomain): String {
        logger.info("c=ConfirmedCustomerCommunicationTemplate m=buildCommunicationBody() s=Start userId=${user.identifier}")
        val token = createTokenUseCase.execute(
            user = user.toApplicationDTO(),
            tokenType = TypeTokenEnum.TOKEN_CONFIRMATION_ACCOUNT.value
        )
        ...
        logger.info("c=ConfirmedCustomerCommunicationTemplate m=buildCommunicationBody() s=Done userId=${user.identifier}")
        return body
    }
    ...
    override fun defineNotificationType(): String {
        return TypeNotificationEnum.SEND_CUSTOMER_CONFIRMATION.value
    }
}